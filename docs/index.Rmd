...
### Perceived Economic Situation (work in progress)
...
dygraph(economic_sentiment,group = "keywords")%>%
  ...
  dySeries("value", label = "Index")%>%
    ...
    dyEvent("2020-9-21", "Restrictions gastronomy and events", labelLoc = "bottom")%>%
    ...
    dyEvent("2020-2-7", "End of Lockdown #3", labelLoc = "bottom")%>%
...
dygraph(clothing,group = "keywords")%>%
  dyAxis("x", drawGrid = FALSE)%>%
  dySeries("value", label = "Index")%>%
    dyEvent("2020-3-13", "Lockdown #1", labelLoc = "bottom")%>%
      dyRangeSelector(dateWindow = c("2020-01-01", today))%>%
      dyOptions(useDataTimezone = TRUE)
```
